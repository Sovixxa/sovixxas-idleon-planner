...
let previousRaw,settings=defaults(),water=0,payment='all',compressed=false,checklistSignature='',completed=[],undoCounts=[];
...
function render(host,raw){
 if(raw!==previousRaw){previousRaw=raw;settings=defaults();water=0;payment='all';compressed=false;}
 ...
 const draw=()=>{
  ...
  const rows=M.recommendationRows(result.steps,{completed,payment,compress:compressed});
  ...
  host.querySelector('[data-compress]').onchange=e=>{compressed=e.target.checked;draw();};
