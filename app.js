(()=>{
  ...
  function activityFor(target){
    ...
    if(/farm|crop|garden/.test(raw))return {label:'Farming',icon:'🌱',kind:'farming'};
    if(/spelunk/.test(raw))return {label:'Spelunking',icon:'⛏',kind:'spelunking'};
    if(/divinity/.test(raw))return {label:'Divinity',icon:'✦',kind:'divinity'};
    if(/laboratory|lab/.test(raw))return {label:'Laboratory',icon:'⬡',kind:'laboratory'};
    if(!raw||raw==='none')return {label:'Idle',icon:'☾',kind:'idle'};
    return {label:'Fighting',icon:'⚔',kind:'fighting'};
  }
  function characterRows(){
    ...
    return ids.map(id=>{let stuff=data['PlayerStuff_'+id];if(typeof stuff==='string'){try{stuff=JSON.parse(stuff);}catch(_){stuff=[];}}if(!Array.isArray(stuff))stuff=[];const lv=data['Lv0_'+id],level=Array.isArray(lv)?Math.round(Number(lv[0])||0):null;
      const rawTarget=data['AFKtarget_'+id],activity=activityFor(rawTarget),world=String(rawTarget||'').match(/^w(\d+)/i)?.[1];return {id,name:names[id]||`Character ${id+1}`,target:humanTarget(rawTarget),afk:afkDuration(stuff[0]),level,classId:data['CharacterClass_'+id],classIcon:classIcon(data['CharacterClass_'+id]),activity,world};
    });
  }
  function renderHome(){
  ...
  function renderWorldPage(name){
    ...
    if(name==='quests'){window.QuestsPage.render($('worldContent'),loadedExport||state?.rawRoot||{});return;}
