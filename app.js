(()=>{
  ...
  function renderWorldPage(name){
    ...
    if(name==='quests'){const host=$('worldContent'),rawRoot=loadedExport||state?.rawRoot||{};host.innerHTML='<section class="bonus-system-empty"><h2>Loading quests…</h2><p>Quest progress is being decoded in the background.</p></section>';window.QuestsPage.prepare(rawRoot).then(()=>{if(host.dataset.page==='quests')window.QuestsPage.render(host,rawRoot);}).catch(error=>{if(host.dataset.page==='quests')host.innerHTML=`<section class="bonus-system-empty"><h2>Could not decode quests</h2><p>${esc(error?.message||String(error))}</p></section>`;});return;}
