...
const models=b.BonusSystems.systems(save),emp=models.get('emperor');for(let kill=0;kill<96;kill++){const clone={...emp,emperorKills:kill};const rows=b.ProgressionModels.build(new Map([['emperor',clone]])).emperorBonuses;assert.equal(rows.filter(x=>x.next).length,1,'exactly one next-kill reward at '+kill);}
