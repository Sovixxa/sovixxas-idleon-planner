...
try {
  ...
  const names=[...new Set([...client.matchAll(/assets%2Fdata%2F((?:WorshipTop[0-7]|TotMini[A-D]?|Soul[1-8])\.png)/g)].map(m=>m[1]))];
  ...
  fs.writeFileSync(path.resolve(ROOT,'worship-assets.js'),'window.WORSHIP_ASSETS='+JSON.stringify(names)+';\n');
