...
const fmt=n=>!Number.isFinite(n)?'—':Math.abs(n)>=1e9||(n!==0&&Math.abs(n)<.01)?n.toExponential(3):n.toLocaleString(undefined,{maximumFractionDigits:2});
...
function impactMarkup(m,rate,points,id,state){
 if(state?.raw){
  const v=state.impacts?.[id];
  if(!v)return '<div class="mastery-point-impact" role="status"><b>Account impact</b><small>Calculating account totals…</small></div>';
  if(v.unavailable)return `<div class="mastery-point-impact"><b>Account preview unavailable</b><small>${esc(v.reason)}</small></div>`;
  const unit=esc(v.unit||''),delta=v.test-v.saved;
  return `<div class="mastery-point-impact"><b>${esc(v.label)}</b><small>Saved ${fmt(v.saved)}${unit} → Test ${fmt(v.test)}${unit}</small><small>${signed(delta)}${unit} vs save · ${signed(v.relative)}%${v.lower?' · lower is better':''}</small><small>Next +1 point → ${v.next===null?'Point limit':fmt(v.next)+unit+' ('+signed(v.nextRelative)+'%)'}</small>${v.note?`<small>${esc(v.note)}</small>`:''}</div>`;
 }
...
// Patch changed text/attributes without replacing live controls or unchanged preview nodes.
function patchMarkup(target,html){
 const template=target.ownerDocument.createElement('template');template.innerHTML=html;
 const sync=(to,from)=>{
  if(to.nodeType!==from.nodeType||to.nodeName!==from.nodeName){to.replaceWith(from.cloneNode(true));return;}
  if(to.nodeType===3){if(to.data!==from.data)to.data=from.data;return;}
  if(to.nodeType!==1)return;
  for(const a of [...to.attributes])if(!from.hasAttribute(a.name))to.removeAttribute(a.name);
  for(const a of from.attributes)if(to.getAttribute(a.name)!==a.value)to.setAttribute(a.name,a.value);
  if(to.tagName==='INPUT'&&to.value!==from.value)to.value=from.value;
  const old=[...to.childNodes],next=[...from.childNodes];
  for(let i=0;i<Math.max(old.length,next.length);i++){if(!next[i])old[i].remove();else if(!old[i])to.append(next[i].cloneNode(true));else sync(old[i],next[i]);}
 };
 const old=[...target.childNodes],next=[...template.content.childNodes];
 for(let i=0;i<Math.max(old.length,next.length);i++){if(!next[i])old[i].remove();else if(!old[i])target.append(next[i].cloneNode(true));else sync(old[i],next[i]);}
}
function render(host,scenario,rate,state={}){
 ...
 const impactContext=scenario.id+':'+rate.id;
 if(state.impactContext!==impactContext){state.impacts={};state.impactContext=impactContext;}
 const impactRequest={scenario:scenario.id,character:rate.id,points:{...yp},ids:visible.map(n=>n.id)};
 if(color==='yellow')state.wantedImpactKey=JSON.stringify(impactRequest);impactRequest.key=state.wantedImpactKey;
 ...
 <div class="mastery-actions"><button id="masteryLoad">${color==='yellow'?'Load recommended setup':'Allocate remaining EXP points'}</button><button id="masteryReset">Reset to save</button><button id="masteryClear">Clear points</button><small id="masteryCalcStatus" role="status">Preview only · no save changes</small></div>
 ...
 host.querySelectorAll('[data-master-point]').forEach(el=>el.onchange=()=>{const v=Number(el.value);if(Number.isInteger(v)&&v>=0&&v<=2000)points[Number(el.dataset.masterPoint)]=v;state.patchNext=true;repaint();});
 ...
 if(color==='yellow'&&state.raw&&typeof root.Worker==='function'){
  state.impactRefresh=()=>{
   if(!host.isConnected)return;
   const status=host.querySelector('#masteryCalcStatus');if(status)status.textContent='Preview only · no save changes';
   for(const input of host.querySelectorAll('[data-master-point]')){
    const id=Number(input.dataset.masterPoint),box=input.closest('.mastery-edit-row')?.querySelector('.mastery-point-impact');
    if(box)patchMarkup(box.parentElement,box.parentElement.innerHTML.replace(box.outerHTML,impactMarkup(m,rate,state.yellow,id,state)));
   }
  };
  state.impactDesired=impactRequest;
  if(!state.impactWorker){
   state.impactWorker=new root.Worker('cooking-impact-worker.js');state.impactInitialized=false;
   state.impactWorker.onmessage=e=>{
    state.impactBusy=false;const result=e.data;
    if(result.key===state.wantedImpactKey){state.impactKey=result.key;state.impacts={...state.impacts,...(result.impacts||Object.fromEntries(state.impactDesired.ids.map(id=>[id,{unavailable:true,reason:result.error||'Calculation unavailable.'}])))};state.impactRefresh();}
    else state.sendImpact();
   };
   state.impactWorker.onerror=()=>{state.impactBusy=false;state.impactKey=state.wantedImpactKey;state.impacts=Object.fromEntries(state.impactDesired.ids.map(id=>[id,{unavailable:true,reason:'Account calculation failed. Reopen Cooking to retry.'}]));state.impactRefresh();};
  }
  state.sendImpact=()=>{
   if(state.impactBusy||state.impactKey===state.wantedImpactKey)return;
   state.impactBusy=true;const status=host.querySelector('#masteryCalcStatus');if(status)status.textContent='Updating account values…';const q={...state.impactDesired};if(!state.impactInitialized){q.raw=state.raw;state.impactInitialized=true;}state.impactWorker.postMessage(q);
  };
  clearTimeout(state.impactTimer);state.impactTimer=setTimeout(state.sendImpact,150);
 }
