(function(root){
  ...
  function render(host,data,rawRoot={}){
    ...
    let tab='Meals',page=0,setup=false;
    ...
    function paint(){
      ...
      host.innerHTML=`<div class="section-head compact"><div><p class="eyebrow">World 4</p><h2>Cooking</h2></div><span class="muted">${model.levels.slice(0,catalog.length).filter(v=>v>0).length}/${catalog.length} meals · Cap ${config.cap}${capConfirmed?'':' (check)'}</span></div>
        <div class="cook-toolbar"><div class="cook-tabs" role="tablist" aria-label="Cooking sections">${['Meals','NMLB'].map(t=>`<button role="tab" aria-selected="${tab===t}" data-cook-tab="${t}">${t}</button>`).join('')}</div><button id="cookSetup">${setup?'Hide settings':'Estimate settings'}</button>${tab==='Meals'?`<div class="cook-pages"><button id="cookPrev" aria-label="Previous meal page" ${page===0?'disabled':''}>‹</button><span>Page ${page+1} / 2</span><button id="cookNext" aria-label="Next meal page" ${page===1?'disabled':''}>›</button></div>`:''}</div>
