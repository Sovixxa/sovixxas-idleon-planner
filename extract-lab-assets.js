...
try {
  ...
  const names=[...new Set([...client.matchAll(/assets%2Fdata%2F((?:ConsoleChip[0-9]+|ConsoleJwl[0-9]+|LabBonus[0-9]+)\.png)/g)].map(m=>m[1]))];
  ...
  fs.writeFileSync(path.resolve(ROOT,'lab-assets.js'),'window.LAB_ASSETS='+JSON.stringify(names)+';\n');
...
const lists={};
for(const name of ['ChipDesc','JewelDesc','LabMainBonus']){const start=client.indexOf(name+'=function()');const end=client.indexOf(']}',start);const ctx={};vm.runInNewContext('var '+client.slice(start,end+2),ctx);lists[name]=ctx[name]();}
fs.writeFileSync(path.resolve(ROOT,'lab-data.js'),'window.LAB_CATALOG='+JSON.stringify(lists)+';\n');
